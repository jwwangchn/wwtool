...
class XVIEW2COCO(Convert2COCO):
    ...
    def __xview_parse__(self, label_file, image_file):
        ...
        for line in lines:
            ...
            object_struct['label'] = original_xview_class[label]
...
if __name__ == "__main__":
    ...
    original_xview_class = {}
    converted_xview_class = []
    with open('tools/datasets/xview/xview_class_labels.txt') as f:
        for row in csv.reader(f):
            original_xview_class[row[0].split(":")[1]] = int(row[0].split(":")[0])
            converted_xview_class.append({'supercategory': 'none', 'id': int(row[0].split(":")[0]),  'name': row[0].split(":")[1],                 })
    ...
    if keypoint:
        for idx in range(len(converted_xview_class)):
            converted_xview_class[idx]["keypoints"] = ['top', 'right', 'bottom', 'left']
            converted_xview_class[idx]["skeleton"] = [[1,2], [2,3], [3,4], [4,1]]
    ...
    for imageset in imagesets:
        ...
        xview = XVIEW2COCO(imgpath=imgpath,
                        ...
                        data_categories=converted_xview_class,
                        data_info=info,
