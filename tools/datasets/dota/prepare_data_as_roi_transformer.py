...
from wwtool.datasets.dota import split_with_gt, split_without_gt, GetFileFromThisRootDir, custombasename, parse_dota_poly, parse_dota_poly2
...
def filemove_v2(srcpath, dstpath, extent, num_process=32):
    ...
    filenames = [custombasename(x.strip()) for x in filelist]
...
def extract_largesize_index(labelpath):
    filenames = GetFileFromThisRootDir(labelpath)
    large_size_index = []
    for name in filenames:
        objs = parse_dota_poly(name)
        flag = 0
        for obj in objs:
            ...
            if max_side > 400:
                ...
                break
        if flag:
            large_size_index.append(custombasename(name))
...
def rotate_single_run(name, srcpath, dstpath):
    ...
    objs = parse_dota_poly2(os.path.join(src_labelTxt, name + '.txt'))
...
def rotate_augment(srcpath, dstpath):
    ...
    pool = Pool(32)
    imgnames = GetFileFromThisRootDir(os.path.join(srcpath, 'images'))
    names = [custombasename(x) for x in imgnames]
...
def prepare():
    ...
    split_train = split_with_gt(train_path,
    ...
    split_val = split_with_gt(val_path,
                        os.path.join(data_root_path, 'trainval1024_1'),
                         gap=200,
                         subsize=1024,
                         num_process=args.num_process
    ...
    split_trainval_large = split_with_gt(os.path.join(data_root_path, 'trainval_large'),
    ...
    train_without_balance = GetFileFromThisRootDir(os.path.join(data_root_path, 'labelTxt'))
    train_without_balance_names = [custombasename(x.strip()) for x in train_without_balance]

    # data balance
    with open('./tools/datasets/dota/train_balance_extend.txt', 'r') as f_in:
    ...
    split_test = split_without_gt(os.path.join(test_path, 'images'),
    ...
    test_names = GetFileFromThisRootDir(os.path.join(data_root_path, 'test1024', 'images'))
    test_names = [custombasename(x.strip()) for x in test_names]
...
if __name__ == '__main__':
    prepare()
    # python prepare_data.py --data_path /data/dj/dota_prepare_test --num_process 32