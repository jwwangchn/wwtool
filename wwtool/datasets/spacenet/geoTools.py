...
def geoPolygon2nparray(geo_poly):
    for ring in geo_poly:
        listcord = []
        for pIdx in range(ring.GetPointCount()):
            xPix, yPix, zPix = ring.GetPoint(pIdx)
            listcord.append([xPix, yPix])
        arraycord = np.array(listcord, dtype=np.int32)
...
def readwktcsv(csv_path,removeNoBuildings=True, groundTruthFile=True):
    ...
    buildinglist = []
    #with open(csv_path, 'rb') as csvfile:  ### lwj changed
    with open(csv_path, 'r') as csvfile:
        ...
        for row in building_reader:
            ### polyPix: pixel coordinates of polygons
            if removeNoBuildings:
                if int(row[1]) != -1:
