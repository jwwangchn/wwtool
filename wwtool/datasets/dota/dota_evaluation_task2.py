...
def parse_gt(filename):
    ...
    with open(filename, 'r') as f:
        ...
        for splitline in splitlines:
            ...
            # object_struct['difficult'] = 0
            bbox = [float(_) for _ in splitline[0:8]]
            xmin = round(min(bbox[::2]))
            ymin = round(min(bbox[1::2]))
            xmax = round(max(bbox[::2]))
            ymax = round(max(bbox[1::2]))
...
def main():
    ...
    prpath = r'./results/submit/dota/v310/valset/pr/bbox'
    if os.path.exists(prpath) == False:
        os.makedirs(prpath)
    detpath = r'./results/dota_v207/merge_dota_bbox/Task2_{:s}.txt'
    ...
    for classname in classnames:
        ...
        # plt.plot(rec, prec)
        # plt.savefig(os.path.join(prpath, "{}.pdf".format(classname)))
        # plt.show()
    map = map/len(classnames)
    print('map:', map)
    classaps = 100 * np.array(classaps)
    print('classaps: ', classaps/100.0)
