...
def generate_gaussian_image(height, width, scale=2.5):
    ...
    grayscale_image = np.clip((scaled_gaussian_prob * 127 + 128), 0, 255)
...
def generate_centerness_image(height, width):
    ...
    centerness_image = np.clip(centerness_image * 255, 0, 255).astype(np.uint8)
...
def generate_ellipse_image(height, width):
    ...
    ellipse_image = np.clip(ellipse_image * 255, 0, 255).astype(np.uint8)
