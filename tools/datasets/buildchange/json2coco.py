...
class SIMPLETXT2COCO():
    def __init__(self, 
        ...
        for city in cities:
            for label_fn in os.listdir(os.path.join(self.imgpath, city, 'labels')):
                basename = wwtool.get_basename(label_fn)
                self.imgpaths.append(os.path.join(self.imgpath, city, 'images', basename + '.png'))
                self.annotpaths.append(os.path.join(self.imgpath, city, 'labels_json', basename + '.json'))
...
if __name__ == "__main__":
    ...
    anno_name = [core_dataset_name, release_version, 'trainval', 'roof_footprint']
