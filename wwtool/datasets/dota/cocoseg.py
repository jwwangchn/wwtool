...
def segmentationToCocoResult(labelMap, imgId, stuffStartId=1):
    '''
...
def pngToCocoResult(pngPath, imgId, stuffStartId=1):
    '''
...
def getCMap(stuffStartId=0, stuffEndId=15, cmapName='Paired', addThings=False, addUnlabeled=False, addOther=False):
    '''
    ...
    # cmap = np.array(range(stuffEndId + 1)).reshape(stuffEndId + 1, 1)/255.0
    # cmap = np.hstack((cmap, cmap, cmap))
